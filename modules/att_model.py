...
class AttModel(CaptionModel):
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 1) # change beam size to 1 to make the input fixed. default = 10
        group_size = opt.get('group_size', 1)
        sample_n = opt.get('sample_n', 1) # default = 10
        ...
        # first step, feed bos
        it = fc_feats.new_full([batch_size], self.bos_idx, dtype=torch.long) # it can be longer sequences
